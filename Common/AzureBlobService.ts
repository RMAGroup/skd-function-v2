...
export class AzureBlobService<ContainerNameType extends string> {
    ...
    async blobExists(containerName: ContainerNameType,blobName: string): Promise<boolean> {
        const blobClient = this.blobServicClient.getContainerClient(containerName).getBlobClient(blobName);
